...
class DistilWhisperSTT(BaseSTT):
    """
    Concrete adapter for the Distil Whisper Speech-To-Text engine using faster-whisper.
    Normalizes input PCM bytes, performs transcription, and supports warm-up execution.
    """

    def __init__(self, config: Dict[str, Any]):
        """
        Initializes the Distil Whisper model using configuration parameters.
        ...
        stt_config = models_meta.get("stt_providers", {}).get("distil_whisper", {}) or self.config

        self.model_size = stt_config.get("model_size", "distil-large-v3")
        ...
        logger.info(
            f"Loading Distil Whisper model: size={self.model_size}, "
        ...
        try:
            ...
            print("Distil Whisper Initialized")
            # Perform one-time lightweight inference warm-up using 1 second of silence
            self._warm_up()
        except Exception as e:
            logger.exception(f"Failed to initialize Distil Whisper model: {e}")
            self.model = None
    ...
    def _warm_up(self) -> None:
        ...
        try:
            ...
            list(segments)
            logger.info("Distil Whisper model warm-up completed.")
        except Exception as e:
            logger.warning(f"Distil Whisper warm-up execution failed: {e}")
    ...
    def transcribe(self, audio_data: bytes) -> str:
        ...
        if not self.model:
            logger.error("Distil Whisper model not initialized. Unable to transcribe.")
            return ""
        ...
        try:
            ...
            logger.exception(f"Distil Whisper transcription failed: {e}")
            return ""
